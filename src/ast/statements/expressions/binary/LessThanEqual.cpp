...
void LessThanEqual::generate(AssemblyProgram &ap) const
{
    ...
    ap.addLine("    mov eax, 0");   // Zero eax, becuase setle only affects one byte
    ap.addLine("    setle al");      // If ecx <= eax, set to 1, otherwise 0
}

std::string LessThanEqual::getName() const
{
