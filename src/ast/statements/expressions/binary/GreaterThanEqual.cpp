...
void GreaterThanEqual::generate(AssemblyProgram &ap) const
{
    ...
    ap.addLine("    mov eax, 0");   // Zero eax, becuase setge only affects one byte
    ap.addLine("    setge al");     // If ecx >= eax, set to 1, otherwise 0
}

std::string GreaterThanEqual::getName() const
{
